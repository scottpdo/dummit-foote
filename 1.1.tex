...
\begin{proof}
      $1$ is the identity element of $G$. $1^1 = 1$, so $1 \in G$, and by definition $1 \cdot z = z$ for all $z \in \mathbb{C}$. Multiplication is by definition associative, so it remains to be shown that $G$ is closed under multiplication.

      Let $a, b \in G$. It follows that $a^n = 1$ and $b^m = 1$ for some $n, m \in \mathbb{Z}^+$. Then $1 = a^n b^m = (ab)^{nm}$. The product of $ab$ raised to the $nm$ power is $1$, so it is an element of $G$, and thus $G$ is closed under addition.
